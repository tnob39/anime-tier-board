...
async function installBaseFixtures(
  ...
  // Single gate so query-string board URLs cannot slip past globs.
  await page.route("**/*", async (route: Route) => {
    ...
    if (pathname.startsWith("/api/statuses")) {
      if (req.method() === "GET") {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    if (pathname === "/api/boards" || pathname.startsWith("/api/boards/")) {
      ...
      if (method === "PUT") {
        ...
        try {
          options.counters.lastPutBody = req.postDataJSON();
        } catch {
    ...
    if (pathname.includes("accounts.google.com")) {
...
async function seedGuestBoard(page: Page, board: ReturnType<typeof makeBoard>) {
  await page.addInitScript(
    ([key, value]) => {
      try {
        // Drop seasonal client cache so fixture seasonal API is always used.
        ...
        // Reset handoff flight singletons if a prior document leaked into this realm.
        const g = globalThis as {
          __atb582HandoffFlights?: unknown;
          __atb582SharePosted?: unknown;
        };
        delete g.__atb582HandoffFlights;
        delete g.__atb582SharePosted;
...
async function gotoTier(page: Page) {
  // Cache-bust to avoid bfcache restoring a prior handoff UI.
  await page.goto(`/tier?atb582=${Date.now()}`, { waitUntil: "domcontentloaded" });
  await expect(page.getByRole("heading", { name: "今期アニメTier表" })).toBeVisible({
    timeout: 20_000
  });
}

async function blockServiceWorker(page: Page) {
  await page.route("**/sw.js", async (route) => {
    await route.fulfill({ status: 404, body: "no-sw" });
  });
  await page.addInitScript(() => {
    try {
...
async function attachErrorWatch(page: Page, options?: { allowHttpErrorConsole?: boolean }) {
  ...
  page.on("console", (msg) => {
    if (msg.type() === "error") {
      const text = msg.text();
      // Network resource failures (image-proxy 502, mocked 4xx/5xx) are not app pageerrors.
      if (
        /Failed to load resource|Bad Gateway|Internal Server Error|\b50[0-9]\b|\b409\b/i.test(
          text
        )
      ) {
        return;
      }
      if (options?.allowHttpErrorConsole) {
        return;
      }
...
function boardLabelLocator(page: Page, label: string) {
  return page.getByLabel(`${label}の名前`);
}
...
test.describe("tier auth handoff — guest", () => {
  ...
  test("guest board survives reload; login prompt close keeps state/URL/intent", async ({
    ...
    await expect(boardLabelLocator(page, "S-local")).toHaveValue("S-local");

    const beforeUrl = page.url();
    const boardBefore = await page.evaluate((key) => localStorage.getItem(key), STORAGE_KEY);

    await page.getByRole("button", { name: "共有" }).click();
    const dialog = page.getByRole("dialog").filter({ hasText: "ログインが必要です" });
    ...
    await expect(
      page.getByRole("dialog").filter({ hasText: "ログインが必要です" })
    ).toHaveCount(0);
...
test.describe("tier auth handoff — auth return", () => {
  ...
  test("same board: no conflict modal, one share, no required PUT before share", async ({
    ...
    await expect
      ...
      .toBe(1);
    await expect(
      page.getByRole("dialog").filter({ hasText: "保存済みのTier表があります" })
    ).toHaveCount(0);
  ...
  test("conflict: no PUT/POST until choice; remote adopt skips PUT; local two-step uses expectedUpdatedAt", async ({
    ...
    const dialog = page
      .getByRole("dialog")
      .filter({ hasText: "保存済みのTier表があります" });
  ...
  test("GET 500: local kept, no share, Japanese recovery", async ({ page }) => {
    const counters = emptyCounters();
    const errors = await attachErrorWatch(page, { allowHttpErrorConsole: true });
  ...
  test("invalid / expired / season-mismatch intent: no auto PUT/POST", async ({ page }) => {
    ...
    // broken JSON — no handoff auto share / conflict
